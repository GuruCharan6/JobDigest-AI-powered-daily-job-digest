...
const features = [
  { title: 'Skills-based matching, not keyword spam' },
  { title: 'Fresh jobs in your inbox every morning' },
  { title: 'AI-powered profile setup in 3 minutes' },
]
      ...
      <div className="w-full max-w-[440px] relative z-10 animate-fade-in">
        ...
        <div className="bg-card border border-border rounded-xl p-9 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
          ...
          <div className="flex flex-col gap-2.5 mb-7">
            {features.map(({ title }) => (
              <div key={title} className="flex items-center gap-2.5 p-2.5 bg-secondary/50 rounded-lg">
                <Check size={14} className="text-emerald-400 flex-shrink-0" />
                <span className="text-xs text-secondary-foreground font-medium">{title}</span>
              </div>
            ))}
          </div>

          <h1 className="text-2xl font-bold text-foreground mb-1 tracking-[-0.02em]">
            Create your account
          </h1>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link to="/login" className="text-primary no-underline font-semibold hover:underline">
            Sign in
          </Link>
        </p>
        <p className="mt-2.5 text-center text-xs text-muted-foreground/60">
          By signing up you agree to our{' '}
          <a href="#" className="text-muted-foreground no-underline font-medium hover:text-foreground transition-colors">Terms</a> &{' '}
          <a href="#" className="text-muted-foreground no-underline font-medium hover:text-foreground transition-colors">Privacy Policy</a>
        </p>
      </div>
    </div>
  )
